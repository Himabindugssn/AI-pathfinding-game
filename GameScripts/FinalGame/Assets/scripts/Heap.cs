...
public class Heap<T> where T : I_heap_item<T> {
	...
	T[] items; //array of T type 
    int cur_item_count=0;

    public Heap(int max_heap_size) // constructor
    {
        items=new T[max_heap_size]; 
    }
    public void Add(T item)  // add an element to the heap
    {
    ...
    */
    ...
    public T top() 
    //returns the element at the root of the heap, and removes it from the heap after returning
    {
        T top_item=items[0];
        cur_item_count--;
    ...
    //returns whether an element is present in the heap or not
    {
    ...
    //swap the item and the index of the elements inside the heap
    {
